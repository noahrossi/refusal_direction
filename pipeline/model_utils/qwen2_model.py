...
# Qwen2/2.5 uses the same chatml template as Qwen1, but the HF model
# architecture is Llama-like (model.model.layers, self_attn, o_proj, down_proj).
...
QWEN2_REFUSAL_TOKS = [40, 2121]  # ['I', 'As'] — same IDs as Qwen1 tokenizer
...
# Qwen2/2.5 uses the same module layout as Llama3:
...
class Qwen2Model(ModelBase):
    ...
    def _load_tokenizer(self, model_path):
        ...
        # Qwen2.5 already has pad_token set to <|endoftext|>; use eos if not
    ...
    def _get_eoi_toks(self):
        return self.tokenizer.encode(
            QWEN2_CHAT_TEMPLATE.split("{instruction}")[-1],
    ...
    def _get_act_add_mod_fn(
        self, direction: Float[Tensor, "d_model"], coeff, layer
    ):
        return functools.partial(
